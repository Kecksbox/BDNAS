...
learning_rate = 0.001
topology_update_trust_region = 1 / 100
...
class Layer:
    def __init__(self, dim_input_layer: int, dim_previous_layer: int, dim_out: int):
        ...
        self.layer_norm = tf.keras.layers.LayerNormalization(center=True, scale=True)
...
class Network:
    ...
    def __call__(self, input_batch, sequence, *args, **kwargs):
        ...
        for weight_matrix, connection_mask_matrix, activation_mask, layer_norm, input_nodes_alive, _ in reversed(
            ...
            start = time.time()
            wm_sign = tf.math.sign(weight_matrix.value)
            weight_matrix.value = weight_matrix.value + wm_sign * 0.001
...
train_log_dir = 'logs/gradient_tape/' + current_time + "sparse_node_mask_classic_sgd_stop" + '/train'
