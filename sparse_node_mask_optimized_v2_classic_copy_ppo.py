...
decay_limit = inital_virtual_layer_output_node_alive_prob
...
class Layer:
    ...
    def init_weight_parameter(self):
        return tf.Variable(tf.zeros(shape=(self.num_valid_input_nodes,)))
    ...
    def sample_topologie(self, output_nodes_alive: SparseRowMatrix):
        ...
        weight_bias = 0.0000001
        weight_sign_mask = tf.sign(weight_matrix.value)
        weight_matrix.value = weight_matrix.value + weight_sign_mask * weight_bias
...
def apply_batch(batch, loss_function: Callable, train_weight: bool, train_topology: bool):
    ...
    with tf.GradientTape() as topology_tape:
        ...
        while scale > 0:
            ...
            new_topology_distribution = test.calculate_alive_probabilities()
            # difference = tf.reduce_max(
            #    tf.abs((tf.concat([tf.reshape(e.value, shape=(-1,)) for _, e in new_topology_distribution], axis=0) / topo_dist_backup) - 1)
            # )
            y_true = topo_dist_backup
            y_pred = tf.concat([tf.reshape(e.value, shape=(-1,)) for _, e in new_topology_distribution], axis=0)
