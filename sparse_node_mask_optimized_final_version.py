...
weight_bias = 0.01
inital_conn_param_scale = 1 / 100
inital_conn_param_scale_to_input = 1.0
...
class Layer:
    ...
    def init_connection_parameter(self):
        return tf.Variable(
            tf.concat([
                tf.random.uniform(shape=(self.dim_previous_layer,)) * inital_conn_param_scale,
                tf.random.uniform(shape=(self.dim_input_layer,)) * inital_conn_param_scale_to_input,
            ], axis=0)
        )
        ...
        for weight_matrix, connection_mask_matrix, activation_mask, layer_norm, input_nodes_alive, _ in reversed(
            ...
            if i < sequence_length:
                ...
                activation_std = tf.math.sqrt(
                    tf.reduce_mean(tf.math.square(activation.value - activation_mean), axis=0, keepdims=True) + 0.001)
...
decay_interval = 999999999999999
log_interval = 100
