...
min_conn_param = -8.0
max_conn_param = 8.0
conn_to_virtual_init_param = -3.0
...
class Layer:
    def __init__(self, dim_in: int, dim_out: int, branching_factor: float = 2.0):
        ...
        self.bias_vector = tf.Variable(tf.random.uniform(shape=(dim_out, 1)))
...
class Network:
    ...
    def sample_topologie(self, max_depth, train: bool):
        ...
        while True:
            ...
            sequence.append(
                (weight_matrix, bias, connection_mask_matrix, output_nodes_alive,
            ...
            connection_mask_matrix_reduced_to_nodes = reduce_connection_mask_matrix_operation(connection_mask_matrix,
                                                                                              self.input_layer.dim_in)
            connection_mask_matrix_reduced_to_nodes = tf.ones(shape=connection_mask_matrix_reduced_to_nodes.shape)
    ...
    def __call__(self, input, sequence, *args, **kwargs):
        ...
        i = 1
        for weight_matrix, bias, connection_mask_matrix, output_nodes_alive, _, _ in reversed(sequence):
...
train_log_dir = 'logs/gradient_tape/' + current_time + "dense_full_evaluation" + '/train'
