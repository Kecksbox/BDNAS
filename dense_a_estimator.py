...
min_conn_param = -2.0
max_conn_param = 2.0
conn_to_virtual_init_param = -2.0
conn_to_input_init_param = conn_to_virtual_init_param

activation_histories = [None] * 10
a_estimate_lr = 0.01
...
def binary_gate_sigmoid(params, alive_vector, alive_probs_vector, train):
    ...
    return sample * tf.expand_dims(alive_vector, axis=-1), probs * tf.expand_dims(
        alive_probs_vector, axis=-1)


@tf.custom_gradient
def check(weight_matrix, activation, previous_actvation_estimate):
    def grad(upstream):
...
class Layer:
    def __init__(self, dim_in: int, dim_out: int, branching_factor: float = 2.0):
        ...
        self.connection_parameter = tf.Variable(tf.concat([
            tf.constant(conn_to_virtual_init_param, shape=(dim_out, next_layer_output_dim), dtype=tf.float32),
            tf.constant(conn_to_input_init_param, shape=(dim_out, dim_in), dtype=tf.float32)
        ], axis=-1))
    ...
    def init_connection_parameter(self):
        return tf.constant(conn_to_virtual_init_param, shape=(1, self.num_valid_input_nodes))
    ...
    def sample_topologie(self, output_nodes_alive: tf.Tensor, output_nodes_alive_prob, train):
        ...
        weight_matrix = self.weight_matrix * connection_mask_matrix

        return weight_matrix, connection_mask_matrix, self.bias_vector, connection_probs_matrix
...
class Network:
    ...
    def sample_topologie(self, max_depth, train: bool):
        ...
        depth = 0
        while True:
            weight_matrix, connection_mask_matrix, bias, connection_probs_matrix = current_layer.sample_topologie(
                output_nodes_alive, output_nodes_alive_probs, train)

            sequence.append(
                (weight_matrix, connection_mask_matrix, bias, output_nodes_alive,
    ...
    def __call__(self, input, sequence, *args, **kwargs):
        ...
        i = 1
        for weight_matrix, connection_mask_matrix, bias, output_nodes_alive, _, _ in reversed(sequence):

            activation_mask = reduce_connection_mask_matrix_operation(connection_mask_matrix, num_input_nodes)
            if activation_histories[len(sequence) - i] is None:
                activation_histories[len(sequence) - i] = tf.zeros(shape=activation.shape)
            else:
                activation_histories[len(sequence) - i] += (a_estimate_lr * (
                            activation - activation_histories[len(sequence) - i])) * tf.expand_dims(activation_mask,
                                                                                                    axis=-1)

            previous_activation = activation_histories[len(sequence) - i] * tf.expand_dims(
                tf.cast(tf.logical_not(tf.cast(activation_mask, dtype=tf.bool)), dtype=tf.float32),
                axis=-1) + activation * tf.expand_dims(activation_mask, axis=-1)

            activation = tf.concat([activation, input], axis=0)

            activation = check(weight_matrix, activation, tf.concat([previous_activation, input], axis=0))
            if i < len(sequence):
...
train_log_dir = 'logs/gradient_tape/' + current_time + "dense_a_evaluation" + '/train'
